...
const roleHeading = (job: Job): string => `<div class="compare-role"><strong>${esc(job.title)}</strong><span>${esc(job.companyName)} · ${esc(job.location)}</span>${safeHref(job.url) ? `<a href="${safeHref(job.url)}" target="_blank" rel="noreferrer">Open listing</a>` : ""}</div>`;
...
async function load() {
  ...
  const catalog = (summary.sourceCatalog ?? []) as Array<{ sourceId: string; name: string; url: string; status: string; note: string; sourceFamily: string; collectorId: string | null; oracleId: string | null; scope: { geography: string | null; department: string | null; careerStage: string | null; employmentType: string | null; boardKind: string } }>;
  ...
  jobs.innerHTML = jobsData.length ? jobsData.map((job) => `<article class="job-card" data-id="${esc(job.observationId)}"><div class="job-top"><div><div class="job-title">${esc(job.title)}</div><div class="job-company">${esc(job.companyName)}</div><div class="job-meta">${esc(job.location)} · ${esc(job.dataMode).toUpperCase()} · source confidence ${Math.round((job.sourceConfidence ?? 0) * 100)}%</div>${safeHref(job.url) ? `<a class="job-link" href="${safeHref(job.url)}" target="_blank" rel="noreferrer">Open listing ↗</a>` : ""}</div><div><div class="job-gap">${esc(job.analysis.gapLabel)}</div><div class="job-state">LIFECYCLE · ${esc(job.analysis.lifecycleState)}</div><div class="job-freshness">FRESHNESS · ${esc(job.analysis.freshness.precision)}</div></div></div></article>`).join("") : `<div class="detail empty"><p>No live observations are stored yet. Run a validated Bright Data collector to populate the production database.</p></div>`;
